...
//        virtualVideo.addScene(videoScene);
    ...
    private void initPlayer() {
        ...
        videoViewSdk.setLayoutParams(layoutParams);
...
//        }
        ...
        if (path == null || !new File(path).exists()) {//如果播放路径不存在
            ToastUtil.showToast("视频错误，请重试");
            return;
        }
//        MAsset assetAudio = new MAsset(RecordManager.get().getProductEntity().combineAudio);
//        assetAudio.setSourceType(DCAsset.DCAssetTypeAudio);
        scene.addMedia(asset);
//        scene.addMedia(assetAudio);
        //设置比例显示
        aVideoConfig config = VideoUtils.getMediaInfor(RecordManager.get().getProductEntity().combineVideo);
        KLog.d("Trim-videoAct--path>>" + RecordManager.get().getProductEntity().combineVideo);
        KLog.d("Trim-videoAct-->>" + config.getVideoHeight() + "width:>" + config.getVideoWidth());
        int[] targetSize = PlayerResize.resize(config.getVideoWidth(), config.getVideoHeight());
...
//        videoViewSdk.setLayoutParams(params);


        playerEngine.build(videoViewSdk, new PlayerCreateListener() {
            @Override
    ...
    public void onUploadOk(PublishResponseEntity entity) {
        ...
        if (entity != null && entity.share_info != null) {
            ppwShare = PopupWindowUtils.showUploadResultPanel(this, videoViewSdk, new MyClickListener() {
                @Override
    ...
    private void exitPublish(boolean isSuccess) {
        ...
        videoViewSdk.postDelayed(new Runnable() {
            @Override
            public void run() {
                if (isSuccess) {
    ...
    protected void onResume() {
        super.onResume();
        if (playerEngine != null && !playerEngine.isNull()) {
            if (!isClickPause && viewUploadMask.getVisibility() != View.VISIBLE) {
//                playVideo();
                combinePreview();
            }
        }
