...
//        mHandler.sendEmptyMessageDelayed(MESSAGE_REFRESH_PLAYER, delayedTime);
    ...
    private void setTimeRange(int index) {
        ...
        for (int i = 0; i < mediaObjects.size(); i++) {
            ...
            if (i<mEndTimeArray.length && mEndTimeArray[i] > 0) {
                ...
                shortVideoEntity.setTrimRange(((long) mStartTimeArray[i] ), (long) (mEndTimeArray[i]));
                trimAudio(i,shortVideoEntity.editingAudioPath,(long)mStartTimeArray[i]*1000000,(long)mEndTimeArray[i]*1000000,shortVideoEntity.volume);
            }
        }
        if (playerEngine != null)
            playerEngine.setMediaAndPrepare(mediaObjects);
