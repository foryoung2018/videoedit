...
public class AudioPlayer implements  IPlayComplete {
   ...
   public interface PlayingPositionListener{
    ...
    public long getcurrenttime () {
       ...
       return mPlayOffset /2 *1000 /mAudioParam.frequency;
    ...
    public long  seek(long time) {
       ...
       long  seekTime = 0;
        synchronized (mSyncObject) {
            int postion =  (int)(time /1000 *mAudioParam.frequency);
            seekTime = 1000 * mVoiceBeatingTool.audioSeek(postion) /mAudioParam.frequency;

            mPlayOffset = seekTime/1000 * mAudioParam.frequency * 2;
        }
            ...
            while (true) {
                ...
                try {
                    synchronized (mSyncObject) {
                        ...
                        byte [] data = mVoiceBeatingTool.audioDataCallback(mPrimePlaySize);
                        ...
                        boolean getedFlag = mAudioTrack.getTimestamp(timestamp);
                        Log.e("AudioTrack", "audioTrack timestampx " + timestamp.framePosition * 1000 / mAudioParam.frequency + "   namtime" + timestamp.nanoTime);

                        synchronized (mSyncObject) {
                            mPlayOffset += mPrimePlaySize;
                            timeListener.OnPlayingPosition(mPlayOffset / 2 * 1000 / mAudioParam.frequency);
                        }
