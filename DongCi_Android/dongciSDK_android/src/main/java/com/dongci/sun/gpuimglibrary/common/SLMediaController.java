...
import com.dongci.sun.gpuimglibrary.gles.SLInputSurface;
import com.dongci.sun.gpuimglibrary.gles.SLOutputSurface;
...
public class SLMediaController {
    ...
    public static SLMediaController getInstance() {
        SLMediaController localInstance = Instance;
        if (localInstance == null) {
            synchronized (SLMediaController.class) {
                localInstance = Instance;
                if (localInstance == null) {
                    Instance = localInstance = new SLMediaController();
                }
            }
        }
    ...
    @TargetApi(16)
    private long readAndWriteTrack(MediaExtractor extractor, SLMP4Builder mediaMuxer, MediaCodec.BufferInfo info, long start, long end, File file, boolean isAudio) throws Exception {
    ...
    public boolean convertVideo(final String path, String outPath, OnProgressUpdateListener updateListener) {
        ...
        if (resultWidth != 0 && resultHeight != 0) {
            SLMP4Builder mediaMuxer = null;
            MediaExtractor extractor = null;

            try {
                MediaCodec.BufferInfo info = new MediaCodec.BufferInfo();
                SLMp4Movie movie = new SLMp4Movie();
                movie.setCacheFile(cacheFile);
                movie.setRotation(rotationValue);
                movie.setSize(resultWidth, resultHeight);
                mediaMuxer = new SLMP4Builder().createMovie(movie);
                ...
                if (resultWidth != originalWidth || resultHeight != originalHeight) {
                    ...
                    if (videoIndex >= 0) {
                        ...
                        SLInputSurface inputSurface = null;
                        SLOutputSurface outputSurface = null;
                        ...
                        try {
                            ...
                            if (Build.VERSION.SDK_INT >= 18) {
                                inputSurface = new SLInputSurface(encoder.createInputSurface());
                            ...
                            if (Build.VERSION.SDK_INT >= 18) {
                                outputSurface = new SLOutputSurface();
                            } else {
                                outputSurface = new SLOutputSurface(resultWidth, resultHeight, rotateRender);
                            }
                                                ...
                                                if (!errorWait) {
                                                    if (Build.VERSION.SDK_INT >= 18) {
                                                        ...
                                                        if (inputBufIndex >= 0) {
                                                            ...
                                                            yuvBuf.clear();
                                                            SLCovertUtil.convertVideoFrame(rgbBuf, yuvBuf, colorFormat, resultWidth, resultHeight, padding, swapUV);
