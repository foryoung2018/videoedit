            ...
            if (mPlayer != null && mPlayer.getAssetWrappers() != null) {
                ...
                for (DCAssetWrapper assetWrapper : mPlayer.getAssetWrappers()) {
                    if (assetWrapper.getAsset().type == DCAsset.DCAssetTypeImage) {
                        ...
                        Surface s = new Surface(renderVideo.surface);
        ...
        synchronized (mRenderReleasedObject) {
            ...
            for (DCRenderObject obj : mRenderObjects) {
                if (obj.getClass().equals(DCRenderVideo.class)) {
                    ...
                    if (renderVideo.surface != null && renderVideo.surface.equals(surface)) {
                        renderObject = obj;
                        break;
                    }
                }
        ...
        mMatrixStack.gl_pushMatrix();
        {
            ...
            if (actors != null) {
                for (DCActor actor : actors) {
                    ...
                    DCTimeEvent timeEvent = DCTimeEventManager.timeEventManager().getTimeEvent(timestamp, actor.trackId);
                    if (timeEvent != null) {
                        actor.assetId = timeEvent.assetId;
                    } else if (actor.trackId > 0 && actor.actorId > 0 && !DCScriptManager.scriptManager().currentScript.isAnimated) {
//                        continue;
                    }

                    if (actor.actorId != 0 && actor.assetId == 0 && actor.type.equals(ACTOR_TYPE_VIDEO)) {
                        actor.assetId = 1;
                    }
                    ...
                    if (renderObject != null) {
                        ...
                        if (DCScriptManager.scriptManager().currentScript.timeRange.duration != 0) {
                            if (!DCScriptManager.scriptManager().currentScript.timeRange.containsTime(timestamp)) {
                                continue;
                            }
                        }
                        ...
                        if (DCScriptManager.scriptManager().currentScript.timeRange.containsTime(timestamp) || timeRange.startTime <= timestamp) {
                            ...
                            renderObject.draw(mMatrixStack, timestamp, actor, timeEvent, filter, viewportWidth, viewportHeight);
        ...
        if (mIsForExporting) {
            for (DCRenderObject renderObject : mRenderObjects) {
                if (renderObject.getClass().equals(DCRenderVideo.class)) {
                    ...
                    if (renderVideo.surface != null) {
                        long timestamp = renderVideo.surface.getTimestamp();
