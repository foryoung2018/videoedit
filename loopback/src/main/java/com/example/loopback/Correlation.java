...
public class Correlation implements Parcelable {
    ...
    public double mEstimatedLatencySamples = 0;// 估计延迟
    public double mEstimatedLatencyMs = 0;// 估计延迟
    ...
    public void computeCorrelation(double [] data, int samplingRate) {
        ...
        Log.i("RecordActivitySdk","Maxvalue" + String.format(" Maxvalue %f, max Index : %d/%d (%d)  minIndex = %d", maxValue, maxIndex,
                          mDataAutocorrelated.length, data.length, minIndex));
        Log.i("RecordActivitySdk","average" + String.format("  average : %.3f  rms: %.3f", average, rms));
        ...
        if (average > 0) {
            ...
            Log.i("RecordActivitySdk" , "Raw" + String.format("Raw: %.3f", raw));
        ...
        Log.i("RecordActivitySdk","latencySamples" + String.format(" latencySamples: %.2f  %.2f ms", mEstimatedLatencySamples,
                          mEstimatedLatencyMs));
    ...
    private boolean downsampleData(double [] data, double [] dataDownsampled, double threshold) {
        ...
        Log.i("RecordActivitySdk" , "Threshold" + String.format(" Threshold: %.3f, ignored:%d/%d (%%.2f)",
