...
public class MyAppActivityManager {
    ...
    /**
     * 结束除cls,cls1,之外的所有activity,执行结果都会清空Stack
     * @param cls
     * @param cls1
     */
    public void finishAllActivityExceptTwo(Class<? extends Activity> cls,Class<? extends Activity> cls1) {
        long startTemp = System.currentTimeMillis();
        KLog.i("Time--=130>" + 0);
        Activity push = null;
        while (!activityStack.empty()) {
            KLog.i("Time--=14>" + (System.currentTimeMillis() - startTemp));
            Activity activity = activityStack.pop();
            if(activity==null)
                continue;
            ...
            KLog.i("Time--=15>" + (System.currentTimeMillis() - startTemp));
        }
        activityStack.push(push);
        KLog.i("Time--=16>" + (System.currentTimeMillis() - startTemp));
    }
