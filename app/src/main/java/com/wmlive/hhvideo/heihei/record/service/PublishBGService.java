...
public class PublishBGService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent == null)//系统杀死后 重启
            return super.onStartCommand(intent, flags, startId);
        KLog.i("onStartCommand");
        boolean ifsave = intent.getBooleanExtra("ifsave", false);
        int fromType = intent.getIntExtra("publishType",PublishModel.TYPE_RECORD);
        ...
        if (publishModel == null) {
            publishModel = new PublishModel(fromType);
