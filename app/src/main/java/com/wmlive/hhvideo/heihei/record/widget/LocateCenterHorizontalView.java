...
public class LocateCenterHorizontalView extends RecyclerView {
    ...
    private boolean isFirstPosChanged = false;        //刚初始化时是否触发位置改变的监听
    ...
    public void moveToPosition(int position) {
        ...
        int itemWidth = wrapAdapter.getItemWidth();
        KLog.i("===yang  x position " + position + " | " +selectPos  + "itemWidth:" + itemWidth);
        if (position != selectPos) {
            int deltx = (position - selectPos) * itemWidth;
            selectPos = position;
            mScroller.startScroll(getScrollX(), getScrollY(), deltx, 0);
            KLog.i("===yang  x scroll " + getScrollX() + " | " +getScrollY() +" | " + deltx);
            postInvalidate();
        }
    }
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            oldMoveX += x;
            KLog.i("===yang  x " + x);
            KLog.i("===yang  x oldMoveX " + oldMoveX);
            scrollBy(x, 0);
...
//        calculateSelectedPos();
    ...
    private void calculateSelectedPos() {
        ...
        KLog.i("===yang  x selectPos" + selectPos);
    }
    ...
    class WrapperAdapter extends RecyclerView.Adapter {
        ...
        public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            if (viewType == HEADER_FOOTER_TYPE) {
                ...
                RecyclerView.LayoutParams params = new LayoutParams(headerFooterWidth, ViewGroup.LayoutParams.WRAP_CONTENT);
            ...
            KLog.i("===yang  x width" + width +" | " + parent.getMeasuredWidth() + parent );
            if(itemView!=null){
