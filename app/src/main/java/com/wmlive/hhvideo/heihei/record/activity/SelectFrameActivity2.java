...
//        DCLatencytestTool.startLatency(context);
    ...
    public static void startSelectFrameActivity2(final BaseCompatActivity context, final byte importType, TopicInfoEntity topicInfo, final long opusId, final String frameLayout) {
    ...
    private void initFrame() {
        ...
        ztab.setOnTabSelectedListener(new ZTabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(int position) {
                ...
                int frameIndex = getFrameIndex(currentFramName, frameList);
                if (mainDownloadMap != null && mainDownloadMap.size() > 0) {
                    frameAdapter.setInitPosition(frameIndex, mainDownloadMap.size());
                } else {
                    frameAdapter.setInitPosition(frameIndex, -1);
                }
                if (frameIndex > -1)
                    rvFrames.smoothScrollToPosition(frameIndex);
            }
        ...
        if (f != null) {
            ...
            if (topicInfo != null && !TextUtils.isEmpty(topicInfo.topicTitle)) {
                ...
                KLog.d("topicInfo", "initFrame: topicInfo==" + topicInfo);
            }
            frameAdapter.addData(frameList);
        }
    }
