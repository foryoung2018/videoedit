...
//        return null;
    ...
    public static void transformAudio2to1(String path, VideoListener videoListener) {
        if (videoListener == null || path== null)
            return;
        videoListener.onStart();
        new Thread(new Runnable() {
            @Override
            public void run() {
                String outpath = null;
                if (!TextUtils.isEmpty(path)) {//本地视频
                    outpath = transformAudio2to1(path);
                }
                ...
                msg.obj = videoListener;
                if(outpath==null){
                    msg.arg1 = SdkConstant.RESULT_CORE_ERROR_ENCODE_VIDEO;
                }else {
                    msg.arg1 = SdkConstant.RESULT_SUCCESS;
                }
                msg.setData(bundle);
                handler.sendMessage(msg);
            }
    ...
    public static String transformAudio2to1(String path) {
        ...
        try {
            ...
            KLog.i(path + "channelCount---pre>"+mediaInfo.audioInfo.channelCount);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 2://2 ->1
                    ...
                    break;
                case 3://2->1  单个视频处理
                    if(msg.arg1==SdkConstant.RESULT_SUCCESS){
                        Bundle bundle = msg.getData();
                        String path = bundle.getString("url");
                        ((VideoListener) msg.obj).onFinish(SdkConstant.RESULT_SUCCESS, path);
                    }else {
                        ((VideoListener) msg.obj).onError();
                    }
                    break;
...
//                releaseExport();
    ...
    public void splitVideoAudioSingle(List<CutEntity> list, VideoListener videoListener) {
    ...
    @Override
    public void export(List<Scene> list, VideoConfigImpl videoConfig,String color, ExportListener exportListener) {
        if (list == null) {
            KLog.e("没视频导出鸡毛");
            return;
        }
...
//        mediaInfo.audioInfo = null;//只有视频
        ...
        getDcVideoExportManager().export(dcScenes, mediaInfo,color, new DCPlayerListener() {
            @Override
            public void onPrepared() {
                KLog.d("export--onPrepared>");
                exportListener.onExportStart();
...
//                        }
    ...
    public static void getMediaInfo(String path, MVideoConfig videoConfig) {
        ...
        if(!"0".equals(rotation)){
            height = Integer.parseInt(retr.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH)); // 视频宽度
            width = Integer.parseInt(retr.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT)); // 视频高度
        }
