...
//            long d = (long) VirtualVideo.getMediaInfo(editingVideoPath, new VideoConfig());
    ...
    public boolean hasRecordMV(){
        KLog.i("hasRecordMv-->"+originalId);
        KLog.i("hasRecordMv--2>"+editingVideoPath);
        return extendInfo.extendName.equals("2") && hasEditingVideo();// 2 本地录制，存在视频
    }
    ...
    public boolean hasEditingAudio() {
        return !TextUtils.isEmpty(editingAudioPath) && new File(editingAudioPath).exists();
    }
    ...
    public boolean hasVideoNoImg(){
        if(hasEditingVideo() && !extendInfo.hasImg){//有视频数据，但是没有图片
            return true;
        }
        return false;
    }
