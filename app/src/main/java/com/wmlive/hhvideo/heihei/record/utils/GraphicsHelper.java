...
public class GraphicsHelper {
     ...
     * @return
     */
    public static String createWatermark(String dcNum) {
        Resources resources = DCApplication.getDCApp().getResources();
//        130 50
        Bitmap output = Bitmap.createBitmap((int)(RecordSetting.WATERMARK_HEIGHT*1.1), RecordSetting.WATERMARK_HEIGHT, Config.ARGB_8888);
        Canvas canvas = new Canvas(output);
...
//        textPaint.setTypeface(typeface);
     ...
     * @return
     */
    public static String createWatermarkNum(String dcNum) {
//        130 50
        Bitmap output = Bitmap.createBitmap((RecordSetting.WATERMARK_HEIGHT), WATERMARK_HEIGHT_ID, Config.ARGB_8888);
        Canvas canvas = new Canvas(output);
        ...
        Typeface typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD);
        textPaint.setTypeface(typeface);
        textPaint.setTextSize(getFontSize(dcNum.length()));
//        textPaint.setStrokeWidth(100f);
//        27  7  9
//        24   8  8
//         24     9  8

        canvas.drawText("ID:" + dcNum, 2, WATERMARK_HEIGHT_ID/2, textPaint);
        // 保存绘图为本地图片
        canvas.save(Canvas.ALL_SAVE_FLAG);
        canvas.restore();
        String watermark = AppCacheFileUtils.getAppImagesPath() + "/dc_watermark.png";
    ...
    private static int getFontSize(int length){
        Resources resources = DCApplication.getDCApp().getResources();
        if(length==7){
            return 26;//resources.getDimensionPixelSize(R.dimen.t9sp);
        }else if(length==8){
            return resources.getDimensionPixelSize(R.dimen.t8sp);
        }else if(length==9){
            return resources.getDimensionPixelSize(R.dimen.t8sp);
        }else if(length>9){
            return resources.getDimensionPixelSize(R.dimen.t7sp);
        }else {
            return resources.getDimensionPixelSize(R.dimen.t9sp);
        }
