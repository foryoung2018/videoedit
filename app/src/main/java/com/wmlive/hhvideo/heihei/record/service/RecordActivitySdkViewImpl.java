...
//        btRecorder.setStartRecord(!isEmpty && videoEntity.canContinueRecord() );
        if(videoEntity!=null && videoEntity.canContinueRecord() ){//该按钮不可以录制
            btRecorder.setStartRecord(false);
        ...
        if(videoEntity!=null)
            KLog.i("resetMenu--btnenable-2>" + btRecorder.isEnabled()+videoEntity.canContinueRecord());
    ...
    public void selectSmallCamera(int index, boolean enableRecord) {
        if (index < act.smallRecordViewList.size()) {
            ...
            new android.os.Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    act.dcRecorderHelper.switchFilter(videoEntity.getFilterId() % FILTER_LIST.size());
                }
            }, 100);
            ...
            try {
                ...
                act.playerEngine.build(textureView, new PlayerCreateListener() {
                    ...
                    public void playCreated() {
                        ...
                        btRecorder.setEnabled(RecordManager.get().getShortVideoEntity(act.getCurrentPreviewIndex())!=null && RecordManager.get().getShortVideoEntity(act.getCurrentPreviewIndex()).canContinueRecord() );
                        //设置是否可以录制
//                        if(!RecordManager.get().getShortVideoEntity(act.getCurrentPreviewIndex()).reachMax()){
////                            btRecorder.setEnabled(true);
//                        }
                        try {
