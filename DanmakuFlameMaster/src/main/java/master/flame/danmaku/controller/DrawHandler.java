...
public class DrawHandler extends Handler {
    ...
    private static final String TAG = DrawHandler.class.getSimpleName();
    ...
    private void bindView(IDanmakuViewController view) {
        this.mDanmakuView = new WeakReference<>(view);
    }
    ...
    public void handleMessage(Message msg) {
        ...
        switch (what) {
            case PREPARE:
                ...
                if (mParser == null || (mDanmakuView.get() != null && !mDanmakuView.get().isViewReady())) {
            ...
            case SHOW_DANMAKUS:
                ...
                if (quitFlag && mDanmakuView != null && mDanmakuView.get() != null) {
                    mDanmakuView.get().drawDanmakus();
                }
                notifyRendering();
                if (!resume) {
                    break;
    ...
    private void updateInCurrentThread() {
        ...
        d = mDanmakuView.get() != null ? mDanmakuView.get().drawDanmakus() : mCordonTime;
        removeMessages(UPDATE);
    ...
    private static class FrameCallback implements Choreographer.FrameCallback {
        ...
        private WeakReference<DrawHandler> handler;

        public FrameCallback(DrawHandler handler) {
            this.handler = new WeakReference<>(handler);
        }

        @Override
        public void doFrame(long frameTimeNanos) {
            if (handler.get() != null && !handler.get().quitFlag) {
                handler.get().sendEmptyMessage(UPDATE);
            }
        }
    }
    ...
    private void updateInChoreographer() {
        if (quitFlag || mFrameCallback == null) {
            return;
        }
...
//            Log.e("DrawHandler", time+"|d:" + d  + "RemaingTime:" + mRemainingTime + ",gapTime:" + gapTime + ",rtim:" + mRenderingState.consumingTime + ",average:" + averageTime);
    ...
    private void syncTimerIfNeeded() {
        Log.i(TAG, "syncTimerIfNeeded: mDanmakusVisible:" + mDanmakusVisible);
        if (mInWaitingState) {
...
//                + mCordonTime + ",mFrameRefreshingRate:" + mFrameUpdateRate);
    ...
    private void prepare(final Runnable runnable) {
        if (drawTask == null) {
            if (mDanmakuView.get() == null) {
                return;
            }
            drawTask = createDrawTask(mDanmakuView.get().isDanmakuDrawingCacheEnabled(), timer,
                    mDanmakuView.get().getContext(), mDanmakuView.get().getWidth(), mDanmakuView.get().getHeight(),
                    mDanmakuView.get().isHardwareAccelerated(), new IDrawTask.TaskListener() {
                        @Override
                        public void ready() {
    ...
    private IDrawTask createDrawTask(boolean useDrwaingCache, DanmakuTimer timer,
        ...
        IDrawTask task = useDrwaingCache ? new CacheManagingDrawTask(timer, mContext, taskListener)
