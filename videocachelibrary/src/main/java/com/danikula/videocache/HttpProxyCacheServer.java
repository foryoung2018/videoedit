...
public class HttpProxyCacheServer {
    ...
    public static boolean deleteFile(String url) {
        File cacheFile = config.generateCacheFile(url);
        File file = new File(cacheFile.getParentFile(), cacheFile.getName() + ".download");
        if (file.exists()) {
            return file.delete();
        }
        return false;
    ...
    public String getCrc64(String url) {
        ...
        HttpProxyCache proxyCache = clients.getProxyCache();
        if (proxyCache != null) {
            return proxyCache.getSource().toString();
        }
        return null;
    }

    public HttpUrlSource getHttpSource(String url) {
        HttpProxyCacheServerClients clients = null;
        try {
            clients = getClients(url);
        } catch (ProxyCacheException e) {
        ...
        HttpProxyCache proxyCache = clients.getProxyCache();
        if (proxyCache != null) {
            return proxyCache.getSource();
        }
        return null;
    }

    public void shutdownClient(String url) throws ProxyCacheException {
        HttpProxyCacheServerClients httpProxyCacheServerClients = clientsMap.get(url);
