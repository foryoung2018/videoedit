...
public class SLVideoProcessor {
    ...
    public boolean extractAudio(String video, String output) {
        String cmd = "-i " + video + " -vn -c:a pcm_s16le -ar 44100 -ac 1 -y " + output;
        return execute(cmd);
    }
    ...
    public boolean extractImagesFromVideo(String video, int fps, int width, int height, String outputDir) {
        String cmd = "-i " + video + " -filter_complex fps=" + fps + ",scale=" + width + ":" + height + " " + outputDir + "/%02d.png";
        return execute(cmd);
    }

    public boolean changeVolume(String audio,float volume,String output) {
        String cmd = " -i " + audio + " -af volume=" + volume + " -vcodec copy " + " -y " + output ;
        return execute(cmd);
    }
