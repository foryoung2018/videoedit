...
import static com.dongci.sun.gpuimglibrary.gles.TextureRotationUtil.CUBE;
import static com.dongci.sun.gpuimglibrary.gles.TextureRotationUtil.TEXTURE_NO_ROTATION;
...
public class GPUImageFilterGroup extends GPUImageFilter {
    ...
    public int onDraw(final int textureId) {
        return onDraw(textureId, mGLCubeBuffer, mGLTextureFlipBuffer);
    }
    ...
    @SuppressLint("WrongCall")
    @Override
    public int onDraw(final int textureId, final FloatBuffer cubeBuffer, final FloatBuffer textureBuffer) {
...
//                int texId = filter.onDraw(previousTexture);
                GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, mFrameBuffers[i]);
                GLES20.glClearColor(0, 0, 0, 0);
                GLES20.glViewport(0, 0, getOutputWidth(), getOutputHeight());

//                int id = texId > 0 ? texId : previousTexture;
                int id = previousTexture;
                if (i == 0) {
                    filter.onDrawFrame(id, cubeBuffer == null ? mGLCubeBuffer : cubeBuffer, textureBuffer);
                }
                else if (i == size - 1) {
                    filter.onDrawFrame(id, mGLCubeBuffer, (size % 2 == 0) ? mGLTextureFlipBuffer : mGLTextureBuffer);
                }
                else {
     ...
     public int getTextureId() {
        if (mFrameBufferTextures != null && mFrameBufferTextures.length > 0) {
            return mFrameBufferTextures[mFrameBufferTextures.length - 1];
        }
        return 0;
     }
